...
MarketMind 是一款基于 FastAPI 与 Vue 3 架构的前后端分离数字化营销大模型 Agent 协同应用。系统通过云端多 Agent 管道编排，实时检索外部竞品舆情痛点，并对冲召回本地品牌资产库（RAG），一键生成小红书、抖音脚本、公众号等多矩阵渠道的爆款流量宣发语料，将原本复杂且繁琐的人工搜集同行竞品舆情、多平台纯手工撰写及肉眼排查合规风险等工作通过云端工作流与底层算法治理，全面优化成**“商家仅需输入自家产品优势与同行竞品链接”的极简两步操作**，即可批量交付安全可控的爆款文案，可以有效地减少商家在线上运营方面的工作量。

针对大模型在创意生成期普遍存在的 **AI 幻觉（恶意抹黑、品牌侵权风险）**，系统在架构出口整合了工业级 **Guardrail（安全合规护栏）终节点**。
...
* 📊 **V2.0 旗舰版纯线性合规管道拓扑图**：
![V2.0 线性合规工作流拓扑图](assets/competitor_content_analyzer_1.png)
